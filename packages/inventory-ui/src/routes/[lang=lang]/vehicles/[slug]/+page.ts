...
import type { FindMaintenanceGuideForVehicleResponse, FindVehicleImplementsResponse, FindVehiclePartsResponse, FindVehicleResponse } from '$lib/types';
...
export const load = (async ({ params, parent }) => {
	...
	const compatibleImplements = await Fetcher.get<FindVehicleImplementsResponse>(`/api/vehicles/${params.slug}/implements`);
	const compatibleParts = await Fetcher.get<FindVehiclePartsResponse>(`/api/vehicles/${params.slug}/parts`);
	const relatedGuides = await Fetcher.get<FindMaintenanceGuideForVehicleResponse>(`/api/maintenance/guides/vehicle/${params.slug}`);
